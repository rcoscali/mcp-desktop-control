...
> Périmètre : **Windows et Linux** (X11 + Wayland). Les spécificités OS sont
...
pixels réels** via `WindowsBackend` / `LinuxBackend`, sélectionnés par
...
- **Accessibilité optionnelle** : si `pywinauto` (Windows) ou `pyatspi` (Linux)
  est absent, les outils `ui_*` le signalent et on retombe sur vision +
  coordonnées.
